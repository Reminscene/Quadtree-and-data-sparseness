...
with open("D://状态升维马尔可夫计算（四叉树-大案例）//空驶轨迹//23-24空驶轨迹.json",'r',encoding='utf8') as fp:
    line_json = json.load(fp)
    line_gdf = gpd.GeoDataFrame.from_features(line_json["features"])


with open("D://状态升维马尔可夫计算（四叉树-大案例）//网格划分边界//23-24网格.json",'r',encoding='utf8') as fp:
...
'''修改异常值（匹配概率过大的改成平均值+随机数）'''
'''mean = grid_gdf['Matching_Prob'].mean()
for i in range(0, len(grid_gdf)):
    if 0.7 <= grid_gdf.loc[i, 'Matching_Prob'] <= 1:
        grid_gdf.loc[i, 'Matching_Prob'] = mean + 0.1*random.random()

mean = grid_gdf['Matching_Prob'].mean()
print(grid_gdf)
print('网格的平均匹配概率为', mean)'''

"""——————————————————————————————————————————————保存人车匹配概率至文本——————————————————————————————————————————————————"""
'''file = open("D://状态升维马尔可夫计算（四叉树-大案例）//人车匹配概率//23-24人车匹配概率.txt", 'w').close()  # 每次写入前清空文本文件的内容
file_handle = open("D://状态升维马尔可夫计算（四叉树-大案例）//人车匹配概率//23-24人车匹配概率.txt", mode='a')  # 将结果不断写入文本文件中！
k = 0
for i in range(0, len(grid_gdf)):
    file_handle.write(str(k)+","+str(grid_gdf.loc[i, 'Matching_Prob']))
    file_handle.write('\n')  # 写入计算结果
    k = k + 1
...
for i in range(0, len(grid_gdf)):
    print(grid_gdf.loc[i, 'Matching_Prob'])